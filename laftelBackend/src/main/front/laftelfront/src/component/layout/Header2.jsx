...
function Header(props) {
   ...
   useEffect(() => {
      axios.get(`/api/auto/${searchMonter}`, {withCredentials:true})
          .then(res => {
             const {data} = res;
   ...
   return (
       <div>
          <Navbar  className={Styles.HeaderContainer} style={{padding:"30px"}}>
             <Navbar.Brand  href="/" style={{
                color: "purple",
                fontSize: "32px",
                fontStyle: "initial"
             }}>LAFTELPEDIA</Navbar.Brand>
             <div className={`me-auto ${Styles.change_color}`}>
             ...
             </div>
             {
                username
                    ? <NavDropdown title={username+"님 환영합니다!"} id="basic-nav-dropdown">
                       <NavDropdown.Item href="/loginPage">마이페이지</NavDropdown.Item>
